...
function scrollToViewer() {
  requestAnimationFrame(() => {
    ...
    els.viewerPanel.scrollIntoView({ behavior: reduceMotion ? "auto" : "smooth", block: "start" });
  });
}
...
function renderViewer(entry) {
  ...
  const tabs = SECTION_META.map(
    (section) => `
      <button type="button" class="page-tab ${section.id === state.section ? "active" : ""}" data-section="${section.id}" aria-pressed="${section.id === state.section ? "true" : "false"}">
        ${section.label}
      </button>
    `
  ).join("");
  ...
  els.viewerPanel.querySelectorAll("[data-section]").forEach((button) => {
    button.addEventListener("click", () => {
      ...
      renderViewer(entry);
      requestAnimationFrame(() => {
        els.viewerPanel.querySelector(`[data-section="${state.section}"]`)?.focus();
      });
    });
  });
...
function renderPitchMetricSummaryTable(rows) {
  const descriptions = [
    ...
    ["zone%", "ストライクゾーン内へ投げ込めた割合", "ストライクゾーン内の投球数 ÷ 投球座標を取得できた投球数"],
    ["z-swing%", "ゾーン内の投球を打者が振った割合", "ゾーン内スイング数 ÷ ゾーン内投球数"],
  ...
  const notes = descriptions
    .map(
      ([label, meaning, formula]) => `
        <div>
          <dt>${label}</dt>
          <dd>
            <span>意味 ${meaning}</span>
            <span>計算 ${formula}</span>
          </dd>
        </div>
      `
    )
...
async function rerender() {
  ...
  if (entry?.detailPath && !entry._detailLoaded) {
    els.viewerPanel.setAttribute("aria-busy", "true");
  } else {
    els.viewerPanel.removeAttribute("aria-busy");
  }
