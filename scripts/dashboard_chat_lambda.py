...
def call_openai(payload: dict[str, Any], search_context: str) -> str:
    ...
    instructions = "\n".join(
        [
            ...
            "Answer in Japanese.",
            "Except for a one-line error or insufficient-data notice, always start the answer with exactly `### 総評` on the first line. On the next line, write one concise plain Japanese sentence that summarizes the answer; do not prefix this sentence with a hyphen or any bullet marker.",
            "After the 総評 sentence, add 0 to 3 short sections as needed. Write each additional section title as a Markdown heading in the form `### 見出し`, then place its key points below it as top-level hyphen bullets.",
            ...
            "Keep 総評 to exactly one sentence and do not add standalone prose before it. For a player-traits question, phrase it naturally in the form `<選手名>は、○○という選手です。` Put any comparison table under a later section heading, then explain traits and interpretation with the two-level bullet structure.",
