...
def call_openai(payload: dict[str, Any], search_context: str) -> str:
    ...
    instructions = "\n".join(
        [
            ...
            "Answer in Japanese.",
            "Except for a one-line error or insufficient-data notice, always start the answer with exactly `### 総評`.",
            "For a player-traits question, write one or two concise plain Japanese lines under `### 総評` without hyphens or bullet markers: first state the player's main style, pitch usage, or role. If supporting numeric evidence is available, always put it on a separate second plain line instead of combining it with the first. For every other question, write one concise plain Japanese summary sentence without a bullet marker.",
            "After the opening content, add 0 to 3 short sections as needed. Give each section a concrete descriptive Markdown heading beginning with `###`, such as `### 球種・球速` or `### 強み`; never use the literal placeholder `見出し`. Place the section's key points below it as top-level hyphen bullets.",
            ...
            "Never create item/value tables with headers such as 項目, 値, 補足. Write those facts as one or two compact Japanese sentences instead.",
            "Remove columns that would be mostly empty, and do not create one-row tables.",
