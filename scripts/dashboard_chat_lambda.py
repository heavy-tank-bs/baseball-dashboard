...
def call_openai(payload: dict[str, Any], search_context: str) -> str:
    ...
    instructions = "\n".join(
        [
            ...
            "For a player-traits question, write one or two concise plain Japanese lines under `### 総評` without hyphens or bullet markers: first state the player's main style, pitch usage, or role; then, when available, state the strongest supporting numeric fact. For every other question, write one concise plain Japanese summary sentence without a bullet marker.",
            "After the opening content, add 0 to 3 short sections as needed. Write each additional section title as a Markdown heading in the form `### 見出し`, then place its key points below it as top-level hyphen bullets.",
