...
- 単一選手・単一試合の成績は短い文章でまとめ、`項目 / 値 / 補足` の表や1行だけの表は作らない。
- 表は複数の選手、球種、年度、日付などを同じ指標で比較する場合、ランキング、時系列に限定する。
- 詳細比較やランキングを明示的に求められない限り、回答内の表は最大1つにする。
- 定性的な特徴や考察は表に入れず、通常の文章または短い箇条書きで説明する。

ブラウザ側でも、モデルが返した `項目 / 値 / 補足` 形式の表と1行だけの表は、罫線のある表ではなく簡潔な数値一覧へ変換する。空の列は表示前に削除する。
